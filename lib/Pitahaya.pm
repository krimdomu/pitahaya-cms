...
sub startup {
    ...
    #######################################################################
    my @cfg = (
        getcwd() . "/pitahaya.conf",
        "/etc/pitahaya.conf", "/usr/local/etc/pitahaya.conf",
    );
    my $cfg;
    for my $file (@cfg) {
        if ( -f $file ) {
    ...
    if ($cfg) {
        ...
        $self->plugin(
            "database",
            {
                dsn => "DBI:mysql:database="
                  . $self->config->{search}->{sphinx}->{index}
                  . ";host="
                  . $self->config->{search}->{sphinx}->{host}
                  . ";port=9306",
                helper  => "sphinx",
        ...
        $self->plugin(
            "Authentication" => {
                ...
                validate_user => sub {
                    ...
                    $self->app->log->debug(
                        "Validateing user: $username -> $password");
        ...
        $admin_r->get('/dialog/select_page_or_media')
          ->to('admin-dialog#select_page_or_media');

        $admin_r->get('/page/tree/')->to( 'admin#page_tree', node => 'root' );
        $admin_r->get('/page/tree/:node')->to('admin#page_tree');
        $admin_r->get('/page/tree/children/:node')
          ->to('admin#page_tree_children');
        ...
        $admin_r->get('/editor/snippet/:snippet_id')
          ->to('admin#editor_snippet_GET');

        $admin_r->get('/media/tree/')->to( 'admin#media_tree', node => 'root' );
        $admin_r->get('/media/tree/:node')->to('admin#media_tree');
        $admin_r->get('/media/tree/children/:node')
          ->to('admin#media_tree_children');
        ...
        $admin_r->get('/user/tree/children/:node')
          ->to('admin-user#tree_children');
        ...
        $admin_r->get('/desktop/:desktop_action')
          ->to('admin-desktop#action_GET');
