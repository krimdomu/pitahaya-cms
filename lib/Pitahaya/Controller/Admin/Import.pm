...
sub import_into_site {
    ...
    my $page_rs =
      $self->db->resultset("Page")->search( { id => { '>' => 0 } } );
    $page_rs->delete;

    my $page_type_rs =
      $self->db->resultset("PageType")->search( { id => { '>' => 0 } } );
    $page_type_rs->delete;

    my $media_rs =
      $self->db->resultset("Media")->search( { id => { '>' => 0 } } );
    $media_rs->delete;

    my $media_type_rs =
      $self->db->resultset("MediaType")->search( { id => { '>' => 0 } } );
...

    {
        ...
        for my $media_line (@content) {
            ...
            $ref->{content} = "file://"
              . File::Spec->catfile( $self->config->{data_dir},
                $site_o->name, $e_file );
    ...
    {
        remove_tree(
            File::Spec->catfile( "templates", "skin", $site_o->skin ) );
        remove_tree(
            File::Spec->catfile(
                "templates", "layouts", "skin", $site_o->skin
            )
        );
    ...
    {
        remove_tree(
            File::Spec->catfile( "public", "css", "skin", $site_o->skin ) );
        remove_tree(
            File::Spec->catfile( "public", "images", "skin", $site_o->skin ) );
        remove_tree(
            File::Spec->catfile( "public", "js", "skin", $site_o->skin ) );
