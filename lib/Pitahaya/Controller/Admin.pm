...
sub page_POST {
    ...
    if ($ref) {
        if ( exists $ref->{type_name} ) {
            my $type =
              $site_o->page_types->search( { "name" => $ref->{type_name} } )
              ->next;
            if ( !$type ) {
                return $self->render(
                    json =>
                      { ok => Mojo::JSON->false, error => "Unknown page type" },
                    status => 500
                );
            }
            $ref->{type_id} = $type->id;
...
sub media_GET {
    ...
    $self->respond_to(
        json => sub {
            $self->render(
                json => { %{ $page_o->get_data }, %additional_option } );
        },
...
sub media_POST {
    ...
    if ( $self->req->headers->content_type =~ m/json/ ) {
        ...
        if ($ref) {
            ...
            if ( exists $ref->{type_name} ) {
                my $type =
                  $site_o->media_types->search(
                    { "name" => $ref->{type_name} } )->next;
                if ( !$type ) {
                    return $self->render(
                        json => {
                            ok    => Mojo::JSON->false,
    ...
    if ( $self->req->headers->content_type =~ m/multipart\/form\-data/ ) {
        ...
        my $type =
          $site_o->media_types->search( { "name" => $media_type_search } )
          ->next;
        if ( !$type ) {
            return $self->render(
                json =>
                  { ok => Mojo::JSON->false, error => "Unknown media type" },
                status => 500
            );
        }
...
sub _load_extra_info {
    ...
    if ( -f File::Spec->catfile( "templates", "$info_html.html.ep" ) ) {
        ...
        my $base_info_html =
          File::Spec->catfile( "skin", "base", "admin",
            lc( $page_o->type->name ), "info" );
        $self->app->log->debug(
            "Looking for info extension in base: $base_info_html.html.ep");
        if (
            -f File::Spec->catfile(
                $FindBin::RealBin, "..",
                "templates",       "$base_info_html.html.ep"
            )
          )
        {
            $self->stash( "info_page",
                $self->render_to_string($base_info_html) );
        }
    }
}
...
sub _load_extra_tabs {
    ...
    if ( -f File::Spec->catfile( "templates", "$tabs_html.html.ep" ) ) {
        ...
        my $base_tabs_html =
          File::Spec->catfile( "skin", "base", "admin",
            lc( $page_o->type->name ), "tabs" );
        $self->app->log->debug(
            "Looking for tabs extension in base: $base_tabs_html.html.ep");
        if (
            -f File::Spec->catfile(
                $FindBin::RealBin, "..",
                "templates",       "$base_tabs_html.html.ep"
            )
          )
        {
            $self->stash( "custom_tabs",
                $self->render_to_string($base_tabs_html) );
        }
    }
}
...
sub _execute_action {
    ...
    my $site_inc_o =
      $self->_get_site_admin_inc( $self->stash("site"), $page_o );
