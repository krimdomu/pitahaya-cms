define([
	...
	// module:
	//		dijit/tree/ObjectStoreModelNoCache

	return declare("dijit.tree.ObjectStoreModelNoCache", Destroyable, {
		// summary:
			...
			if(this.root){
				...
				when(res,
					lang.hitch(this, function(items){
						...
						if(items.length != 1){
							throw new Error("dijit.tree.ObjectStoreModelNoCache: root query returned " + items.length +
		...
		getChildren: function(/*Object*/ parentItem, /*function(items)*/ onComplete, /*function*/ onError){
			...
			//
			// But be careful if we continue to maintain ObjectStoreModelNoCache as a separate class
			// from Tree, because in that case ObjectStoreModelNoCache can be shared by two trees, and destroying one tree
			...
			//
			// I think for 2.0 though that ObjectStoreModelNoCache should be rolled into Tree itself.
